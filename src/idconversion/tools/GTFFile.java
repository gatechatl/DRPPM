...
public class GTFFile {
	...
	public static void initialize_geneName(String fileName, String input_gene_name) {
		...
		try {
			...
			while (in.ready()) {
				...
				if (split.length > 8) {
					...
					if (gene_name.equals(input_gene_name)) {
						if (!gene_id.equals("")) {
						...
						if (split[2].equals("exon")) {
							...
							transcript2exon_num.put(transcript_id + "." + exon_number, new_exon);
						}
