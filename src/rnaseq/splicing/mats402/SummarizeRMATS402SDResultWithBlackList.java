...
public class SummarizeRMATS402SDResultWithBlackList {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			String IR_limma = "IntronRetention.SD.LIMMA.txt";
			int SE_count = 0;
			int SE_count_gene = 0;
			...
			int SD_IR_count = 0;
			int SD_IR_count_gene = 0;
			int SD_increased_retained_intron_count = 0;
			int SD_decreased_retained_intron_count = 0;
			int SD_increased_retained_intron_count_gene = 0;
			int SD_decreased_retained_intron_count_gene = 0;
			
			int combined_SD_IR_count_gene = 0;
			int combined_SD_increased_retained_intron_count_gene = 0;
			int combined_SD_decreased_retained_intron_count_gene = 0;
			
			HashMap black_list = new HashMap();
			FileInputStream fstream = new FileInputStream(blacklist_file);
			DataInputStream din = new DataInputStream(fstream);
			BufferedReader in = new BufferedReader(new InputStreamReader(din));			
			while (in.ready()) {
				String str = in.readLine();
				black_list.put(str.trim(), str.trim());
			}
			in.close();
			...
			fstream = new FileInputStream(outputPath + "/" + SE);
			...
			while (in.ready()) {
				...
				String gene = split[2].replaceAll("\"", "");				
				all_genes_tested.put(gene, gene);				
				double fdr = new Double(split[19]);
				double incLevel = new Double(split[22]);
				if (fdr <= fdr_cutoff && Math.abs(incLevel) >= psi_cutoff && !black_list.containsKey(gene)) {
			...
			HashMap combined_RI_map = new HashMap();
			HashMap combined_increased_RI_map = new HashMap();
			HashMap combined_decreased_RI_map = new HashMap();
			...
			while (in.ready()) {
				...
				if (fdr <= fdr_cutoff && Math.abs(incLevel) >= psi_cutoff  && !black_list.containsKey(gene)) {
					...
					if (!combined_RI_map.containsKey(gene)) {
						combined_SD_IR_count_gene++;
						combined_RI_map.put(gene, gene);
					}
					RI_count++;
					all_gene.put(gene, gene);
					...
					if (incLevel > 0) {
						...
						if (!combined_increased_RI_map.containsKey(gene)) {
							combined_SD_increased_retained_intron_count_gene++;
							combined_increased_RI_map.put(gene, gene);
						}
					} else {
						...
						if (!combined_decreased_RI_map.containsKey(gene)) {
							combined_SD_decreased_retained_intron_count_gene++;
							combined_decreased_RI_map.put(gene, gene);
						}
					}
				}
			}
			...
			HashMap SD_IR_map = new HashMap();
			HashMap SD_increased_RI_map = new HashMap();
			HashMap SD_decreased_RI_map = new HashMap();
			fstream = new FileInputStream(outputPath + "/" + IR_limma);
			din = new DataInputStream(fstream);
			in = new BufferedReader(new InputStreamReader(din));
			header = in.readLine();
			while (in.ready()) {
				...
				if (fdr < 0.05 && !black_list.containsKey(gene)) {
					SD_IR_count++;
					all_gene.put(gene, gene);
					if (!SD_IR_map.containsKey(gene)) {
						SD_IR_count_gene++;
						SD_IR_map.put(gene, gene);
					}
					if (!combined_RI_map.containsKey(gene)) {
						combined_SD_IR_count_gene++;
						combined_RI_map.put(gene, gene);
					...
					if (log2FC > 0) {
						SD_increased_retained_intron_count++;
						if (!SD_increased_RI_map.containsKey(gene)) {
							SD_increased_retained_intron_count_gene++;
							SD_increased_RI_map.put(gene, gene);
						}
						if (!combined_increased_RI_map.containsKey(gene)) {
							combined_SD_increased_retained_intron_count_gene++;
							combined_increased_RI_map.put(gene, gene);
						...
						SD_decreased_retained_intron_count++;
						if (!SD_decreased_RI_map.containsKey(gene)) {
							SD_decreased_retained_intron_count_gene++;
							SD_decreased_RI_map.put(gene, gene);
						}
						if (!combined_decreased_RI_map.containsKey(gene)) {
							combined_SD_decreased_retained_intron_count_gene++;
							combined_decreased_RI_map.put(gene, gene);
			...
			System.out.println("SD_Increased Retained Intron" + "\t" + SD_increased_retained_intron_count + "\t" + SD_increased_retained_intron_count_gene);
			System.out.println("SD_Decreased Retained Intron" + "\t" + SD_decreased_retained_intron_count + "\t" + SD_decreased_retained_intron_count_gene);
			System.out.println("SD_Total RI" + "\t" + SD_IR_count + "\t" + SD_IR_count_gene);
			
			System.out.println("Total_Combined_Intron_Retained_Genes" + "\t" + combined_SD_IR_count_gene);
			System.out.println("Total_Combined_Increased_Retained_Genes" + "\t" + combined_SD_increased_retained_intron_count_gene);
			System.out.println("Total_Combined_Decreased_Retained_Genes" + "\t" + combined_SD_decreased_retained_intron_count_gene);
			...
			System.out.println("SD_IR_Gene List:\t" + map2string(SD_IR_map));
			System.out.println("SD_increased_IR_map List:\t" + map2string(SD_increased_RI_map));
			System.out.println("SD_decreased_IR_map List:\t" + map2string(SD_decreased_RI_map));
			...
			System.out.println("Black List:\t" + map2string(black_list));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
