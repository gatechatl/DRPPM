 ...
 *
 */
public class SummarizeResultsAfterMATSFilterDisplayGeneList {
		...
		try {
			...
			while (in.ready()) {
				...
				for (int i = 1; i < split.length; i++) {
					...
					if (gene_name_exon_count.containsKey(gene)) {
						...
						missing.put(gene, gene);
						//pass_exon_cutoff = true;
					}
					if (new Double(split[i]) >= fpkm_cutoff && pass_exon_cutoff) {
					//if (new Double(split[i]) >= fpkm_cutoff) {
			...
			for (int i = 0; i < split_inputFilePaths.length; i++) {
				...
				for (String file: files) {				
					...
					String gene_list_str = "";
					Iterator itr2 = count_splicing_gene_pass_fpkm_cutoff.keySet().iterator();
					while (itr2.hasNext()) {
						String gene = (String)itr2.next();
						gene_list_str += gene + ",";
					}
					System.out.print("\t" + count + "\t" + gene_list_str);
					if (file.equals("SE.MATS.ReadsOnTargetAndJunctionCounts.txt.addGeneName.txt.filter.txt")) {
			...
			System.out.println("Total missing genes: " + missing.size());
