 ...
 *
 */
public class JuncSalvagerCombinePSIMatrix {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			String replaceNaNwithThis = args[1];
			String outputFile = args[2];
			String outputFileBlackList = args[3];
			String final_outputFile = args[4];
			int count = 0;
			...
			while (in.ready()) {
				...
				count++;
				if (header_once) {
					FileInputStream fstream_head = new FileInputStream(str);
				...
				File f = new File(str);
				if (f.exists()) {
					String sampleName = f.getName().replaceAll(".SJ.out.tab.psi.txt", "");
