...
public class CombineSplicingDeficiencyFlexIndex {
	...
	public static String parameter_info() {
		return "[metaFile] [index] [outputFile]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			HashMap filtered_sd = new HashMap();
			...
			int meta_file_idx = new Integer(args[1]);
			...
			while (in.ready()) {
				...
				String filePath = split[meta_file_idx];
			...
			while (itr.hasNext()) {
				...
				while (in.ready()) {
					...
					if (!split[0].equals("GeneID")) {
						...
						int num_intron_read = new Integer(split[2]);
						int num_exon_read = new Integer(split[4]);
						double splicing_deficiency = new Double(split[1]);
