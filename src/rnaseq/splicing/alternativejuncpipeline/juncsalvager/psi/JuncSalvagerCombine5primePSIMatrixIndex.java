 ...
 *
 */
public class JuncSalvagerCombine5primePSIMatrixIndex {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				if (f.exists()) {
					...
					while (in_psi.ready()) {
						...
						if (new Double(split_psi[split_psi.length - 3]) == 0.0 || new Double(split_psi[split_psi.length - 4]) == 0.0 || (new Double(split_psi[split_psi.length - 3]) + new Double(split_psi[split_psi.length - 4]) < 10)) {
						...
						out.write("\t" + split_psi[split_psi.length - 6].replaceAll("NaN", replaceNaNwithThis));
					}
					in_psi.close();
					out.write("\n");
				}
			...
			while (itr.hasNext()) {
				...
				double score = new Double(black_list_count) / count; // if more than 10% of the samples have a bad splice junction then it is removed
			...
			while (in.ready()) {
				...
				if (f.exists()) {
					...
					while (in_psi.ready()) {
						...
						if (!black_list_index.containsKey(idx)) {
							String exon = split_psi[0] + "_" + split_psi[1] + "_" + split_psi[2];							
							out_final.write("\t" + split_psi[split_psi.length - 6].replaceAll("NaN", replaceNaNwithThis));
						}
						idx++;
					}
					in_psi.close();
