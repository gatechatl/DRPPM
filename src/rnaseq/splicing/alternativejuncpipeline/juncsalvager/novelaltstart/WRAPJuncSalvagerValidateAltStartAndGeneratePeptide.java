package rnaseq.splicing.alternativejuncpipeline.juncsalvager.novelaltstart;
...
/**
 * Input: GTF file and all the novel alternative start site candidate.  
 * @author gatechatl
 *
 */
public class WRAPJuncSalvagerValidateAltStartAndGeneratePeptide {
	...
	public static String description() {
		return "Filter the novel alt start that overlap with the GTF file.";
	}
	...
	public static void execute(String[] args) {
		...
		try {
			...
			HashMap all_novel_alt_start_summary = new HashMap();			
				...
				if (sampleFolder_file.isDirectory()) {
					...
					if (new_file.exists()) {
						...
						for (File geneFolder_file: gene_files) {
							...
							File f = new File(path + "/AlternativeStartSite.txt");
							if (f.exists()) {
								
								FileWriter fwriter = new FileWriter(path + "/AlternativeStartSite.Filtered.bed");
								...
								while (in.ready()) {
									...
									if (!exons.containsKey(split[1] + "\t" + split[2] + "\t" + split[3])) {
										...
										if (all_novel_alt_start_summary.containsKey(exon_coord)) {
											LinkedList sample_list = (LinkedList)all_novel_alt_start_summary.get(exon_coord);
											if (!sample_list.contains(sampleFolder_file.getName())) {
												sample_list.add(sampleFolder_file.getName());
											}
											all_novel_alt_start_summary.put(exon_coord, sample_list);
										} else {
								...
								System.out.println("Missing: " + path + "/AlternativeStartSite.txt");
							}
			...
			Iterator itr = all_novel_alt_start_summary.keySet().iterator();
			while (itr.hasNext()) {
				String bed_line = (String)itr.next();
				out_bed.write(bed_line + "\n");
				LinkedList sample_list = (LinkedList)all_novel_alt_start_summary.get(bed_line);
				String sample_txt = "";
