package rnaseq.mapping.tools.flagstat;
...
public class SummarizeFlagStat {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			String[] vals = {"TOTAL_READS", "MAPPED", "NONDUPS_MAPPED", "PERCENT_MAPPED", "PERCENT_DUPS"};
			...
			while (in.ready()) {
				...
				String flagstat_file = str;

				
				
				STATISTICS stats = new STATISTICS();				
				String sampleName = new File(flagstat_file).getName().replaceAll(".flagstat.txt", "");
				stats = getStats(flagstat_file, sampleName);
				//String percent_uniq_map_reads = "";
				stats.createMap();
				out.write(sampleName);
				for (String val: vals) {
					String stat_value = (String)stats.MAP.get(val);
					out.write("\t" + stat_value);
				}
				out.write("\n");
			}
			in.close();
