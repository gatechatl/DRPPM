...
public class CombineHTSEQResultRefGeneOnly {
	...
	public static String parameter_info() {
		return "[bamFileList] [refGeneList] [outputFile]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			String refGeneList = args[1];
			String outputFile = args[2];			
			
			FileWriter fwriter = new FileWriter(outputFile);
			BufferedWriter out = new BufferedWriter(fwriter);			

			HashMap refGene = new HashMap();
			FileInputStream fstream = new FileInputStream(refGeneList);
			DataInputStream din = new DataInputStream(fstream);
			BufferedReader in = new BufferedReader(new InputStreamReader(din));
			while (in.ready()) {
				String str = in.readLine();
				refGene.put(str.trim(), str);
			...
			while (in.ready()) {
				...
				while (in2.ready()) {
					...
					if (!split[0].contains("__")) {
						if (refGene.containsKey(split[0])) {
