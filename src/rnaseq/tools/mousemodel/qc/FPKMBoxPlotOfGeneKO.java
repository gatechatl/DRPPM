...
public class FPKMBoxPlotOfGeneKO {
	...
	public static String boxplot(String[] gene_names, String[] sampleType) {
		...
		for (String geneKO: gene_names) {
			...
			for (int i = 0; i < sampleType.length; i++) {
				...
				if (i == 0) {
					script += sampleType[i] + " = t(allDat[," + sampleType.length + ":(" + sampleType.length + "+" + sampleType[i] + "_length-1)]) * 1e6;\n";
					lastTag = "(" + sampleType.length + "+" + sampleType[i] + "_length";
				} else {
