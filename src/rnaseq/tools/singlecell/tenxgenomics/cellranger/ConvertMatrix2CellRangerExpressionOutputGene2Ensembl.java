...
public class ConvertMatrix2CellRangerExpressionOutputGene2Ensembl {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				if (!no_dup.containsKey(split[0])) {
					no_dup.put(split[0], split[0]);
					if (gtf.geneName2geneID.containsKey(split[0])) {
						gene_idx++;
						gene2index.put(split[0], gene_idx);
						index2gene.put(gene_idx, split[0]);				
						if (gtf.geneName2geneID.containsKey(split[0])) {
							LinkedList list = (LinkedList)gtf.geneName2geneID.get(split[0]);
							String geneID = (String)list.getFirst();
							out_gene.write(geneID + "\t" + split[0] + "\n");
						}
						
						for (int j = 1; j < split.length; j++) {
							double round = new Double(Math.round(new Double(split[j]) * 100000)) / 100000;
							total += round;
