...
/**
 * Convert the matrix into three file similar to what's presented in cell ranger
 * Example: /rgs01/project_space/zhanggrp/AltSpliceAtlas/common/analysis/BALL_Mullighan
 * @author tshaw
 *
 */
public class ConvertMatrix2CellRangerExpressionGeneIDCleanOutput {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				if (gtf.geneid_clean2geneName.containsKey(split[0])) {
					gene_idx++;
					gene2index.put(split[0], gene_idx);
					index2gene.put(gene_idx, split[0]);				
					
					out_gene.write(split[0] + "\t" + gtf.geneid_clean2geneName.get(split[0]) + "\n");
