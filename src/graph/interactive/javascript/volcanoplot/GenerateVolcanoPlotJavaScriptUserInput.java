...
public class GenerateVolcanoPlotJavaScriptUserInput {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				//pval_list.add(Math.round(new Double(pval_value) * 100) / 100 + "");
				pval_list.add(pval_value + "");
				logFC_list.add(split[logFC_index]);
				fdr_list.add(split[fdr_index] + "");
				name_list.add(split[name_index]);
				avg_expr_list.add(Math.round(new Double(split[avg_expr_index]) * 100) / 100 + "");
