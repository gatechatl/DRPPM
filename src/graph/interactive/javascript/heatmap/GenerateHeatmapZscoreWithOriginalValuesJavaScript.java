...
public class GenerateHeatmapZscoreWithOriginalValuesJavaScript {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				orig_expr[total] = genotype_value_simple[i];				
				total++;
			}
			...
			while (in.ready()) {
				...
				for (int i = 1; i < split.length; i++) {
					col[total] = i;
					row[total] = gene_index;
					zscore[total] = Math.floor(new Double(split[i].split(",")[1]) * 100) / 100;
					orig_expr[total] = Math.floor(new Double(split[i].split(",")[0]) * 100) / 100;
					total++;
				}				
			}
			in.close();
			...
			System.out.println(generate_heatmap_script(row, col, zscore, orig_expr, row_header, col_header, genotype_simple, width, height));
	...
	public static String generate_heatmap_script(int[] row, int[] col, double[] zscore, double[] orig_expr, String[] row_header, String[] col_header, String[] genotype, int width, int height) {
		...
		for (int i = 0; i < row.length; i++) {
			data.append("{row:" + row[i] + ", col:" + col[i] + ", value:'" + zscore[i] + "', expr:'" + orig_expr[i] + "'},\n");
		}
		data.append("];\n");
		...
		script.append("                 .style('top', (d3.event.pageY-10) + 'px')\n");
		script.append("                 .select('#value')\n");
		//script.append("                 .text('lables:'+rowLabel[d.row-1]+','+colLabel[d.col-1]+'\\ndata:'+d.value+'\\nrow-col-idx:'+d.col+','+d.row+'\\ncell-xy '+this.x.baseVal.value+', '+this.y.baseVal.value);  \n");
		script.append("                 .html(colLabel[d.col-1]+'<br>MetaInfo: '+genotypeLabel[d.col-1]+'<br>Gene: '+rowLabel[d.row-1]+'<br>FPKM:'+d.expr);  \n");
