...
public class GenerateHeatmapJavaScript {
		...
		try {
			...
			while (in.ready()) {
				...
				for (int i = 1; i < split.length; i++) {
					...
					value[total] = Math.floor(new Double(split[i]) * 100) / 100;
					total++;
				}				
			}
			in.close();
			
			System.out.println(generate_heatmap_script(row, col, value, row_header, col_header));
	...
	public static String generate_heatmap_script(int[] row, int[] col, double[] value, String[] row_header, String[] col_header) {
		...
		script.append("  </select>\n");*/
		script.append("<div id='chart' style='overflow:auto; width:3000px; height:1500px;'></div>\n");
		script.append("\n");
		...
		for (int i = 0; i < row.length; i++) {
			data.append("{row:" + row[i] + ", col:" + col[i] + ", value:'" + value[i] + "'},\n");
		}
		data.append("];\n");
		script.append(data);
		...
		script.append("                 .text(colLabel[d.col-1]+'\\nGene:'+rowLabel[d.row-1]+'\\ndata:'+d.value);  \n");
