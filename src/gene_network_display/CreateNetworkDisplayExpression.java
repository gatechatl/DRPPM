...
public class CreateNetworkDisplayExpression {
	...
	public static String generateNodesComplex(LinkedList kegg_conn_list, LinkedList kegg_list, HashMap expression, HashMap rawExpression, String networkType, int fontSize) {
		...
		while (itr.hasNext()) {
			...
			if (kegg_list.size() != count) {				
				if (expr.equals("0.0")) {
					str += "{ data: { id: '" + key + "', name: '" + key + "', genes: '" + genes + "', weight: " + weight + ", faveShape: '" + faveShape + "', baz: " + size + ", faveColor: '" + edgecolor + "', bwidth: " + bwidth + "}, position:{x: " + new Double(new Double(entry.XCOORD) * 1.5).intValue() + ",y: " + new Double(new Double(entry.YCOORD) * 1.5).intValue() + "}},\n";
				} else {
