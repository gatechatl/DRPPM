...
public class Matrix2Addition {
	...
	public static String description() {
		return "Add a certain number to each value in the matrix";
	}
	public static String type() {
		return "MISC";
	}
	public static String parameter_info() {
		return "[inputMatrixFile] [number to add: 0] [outputMatrixFile]";
	}
		...
		try {
			...
			while (in.ready()) {
				...
				for (int i = 1; i < split.length; i++) {
					double newVal = new Double(split[i]) + baseNum;
					out.write("\t" + newVal);
				}
				out.write("\n");
			}
