 ...
 *
 */
public class NormalizePhosphoAgainstWholeFlex {
		...
		try {
			...
			while (in.ready()) {
				...
				if (map.containsKey(geneName)) {
					...
					double[] pho = new double[numSamples];
					int j = 0;
					if (kinase_annotation_flag) {
						for (int i = split.length - (numSamples + 1); i < split.length - 1; i++) {
					...
					j = 0;
					for (int i = split2.length - numSamples; i < split2.length; i++) {
