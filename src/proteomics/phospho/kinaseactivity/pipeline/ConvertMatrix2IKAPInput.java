...
public class ConvertMatrix2IKAPInput {

	public static String parameter_info() {
		return "[inputPhosphoFile] [inputProteomeFile] [outputFile]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			HashMap whole_proteome = new HashMap();
			fstream = new FileInputStream(inputProteome);
			...
			while (in.ready()) {
				...
				if (split.length > 1) {
					if (split[1].split("\\|").length > 2) {
						...
						whole_proteome.put(accession, wvalue);
					}
				}
			}
