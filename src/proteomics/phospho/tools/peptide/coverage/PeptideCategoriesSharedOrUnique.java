 ...
 *
 */
public class PeptideCategoriesSharedOrUnique {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				String peptide = split[0];
				String pepName = split[1];
				String num_scans = split[14];
				String xCorr = split[5];
				...
				for (int i: group1_samples) {
					group1_vals[n] = new Double(split[22 + i - 1]);
					sample_vals += "\t" + group1_vals[n];
					n++;
				}
				n = 0;
				for (int i: group2_samples) {
					group2_vals[n] = new Double(split[22 + i - 1]);
				...
				if (peptide_shared.containsKey(peptide)) {
					...
					if (!shared.containsKey(peptide + "\t" + type)) {
						...
						out_shared.write(peptide + "\t" + pepName + "\t" + num_scans + "\t" + xCorr + "\t" + start + "\t" + end + sample_vals + "\n");
					}
