...
public class CompareExonCountDistribution {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				if (map.containsKey(geneName) && aveExpr > 2.0 && new Integer((String)map.get(geneName)) > 0) {
	...
	public static String generateXYPlot(String inputFile, String outputFile, String xlab, String ylab) {
		...
		script += "d = d + geom_point() + geom_smooth(method=\"loess\") + scale_x_log10(breaks=c(2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048)) + theme(text=element_text(size=30)) \n";
