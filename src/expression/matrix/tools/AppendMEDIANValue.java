...
public class AppendMEDIANValue {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			LinkedList median_score_list = new LinkedList();
			
			//LinkedList line_list = new LinkedList();
			out.write(header + "\tMEDIAN\n");
			int count = 0;
			...
			while (in.ready()) {
				...
				if (group1_val.size() == group1_index_list.size()) {
					...
					double median_score = MathTools.median(group1_val);
					median_score_list.add(median_score);
					out.write(str + "\t" + median_score + "\n");
