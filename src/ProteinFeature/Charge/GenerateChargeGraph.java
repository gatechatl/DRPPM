 ...
 *
 */
public class GenerateChargeGraph {

	public static String type() {
		return "PROTEINFEATURE";
	}
	public static String description() {
		return "Generate a graph of the charge and show the region that obtained a hit";
	}
	...
	public static void execute(String[] args) {		
		try {
			...
			HashMap coord = grabCoord(accession_file, tag_flag);			
			...
			while (itr.hasNext()) {
	            ...
	            String coord_str = (String)coord.get(accession);
	            int start = new Integer(coord_str.split("\t")[0]);
	            int end = new Integer(coord_str.split("\t")[1]);
				...
				out.close();
				File f2 = new File(outputImg);
				if (f2.exists()) {
					String newFileName = outputImg;
					int id = 2;
					File f3 = new File(newFileName);					
					while (f3.exists()) {
						newFileName = outputImg + "_" + id + ".png";
						id++;
						f3 = new File(newFileName);
					}
					outputImg = newFileName;
				}
	...
	public static HashMap grabCoord(String inputFile, String tag) {
		...
		try {
			...
			while (in.ready()) {
				...
				if (split[0].equals(tag)) {
					map.put(split[2], split[3] + "\t" + split[4]);
				}
			}
			in.close();
