...
public class EXONCAPHumanBasicStats {

	public static String type() {
		return "EXONCAP";
	}
	public static String description() {
		return "Generate the basic statistics for exoncap, human SNPDetect Output. For mouse, the output is slightly different.";
	}
	...
	public static void generateMutTypeFiles(String inputFile, String sampleFile, String muttype, String output1, String output2, String output3, String output4) {
		...
		try {
			...
			out.write(header + "\tMutType\tSampleType\tTsOrTv\tSampleNameType" + "\n");
			while (in.ready()) {
				String str = in.readLine();
				String[] split = str.split("\t");
				...
				int Ref_MutIndex = findCol(colnames, "Ref>Mut");
				String ref_allele = split[Ref_MutIndex].split(">")[0];
				String mut_allele = split[Ref_MutIndex].split(">")[1];
				
				int ManualReviewFlagIndex = findCol(colnames, "ManualReviewFlag");
				String manReviewFlag = "Good"; 
				if (ManualReviewFlagIndex >= 0) {
					manReviewFlag = split[ManualReviewFlagIndex];					
				}
				
				int MutTumorIndex = findCol(colnames, "MutTumor");
				String MutTumor = "Good"; 
				if (MutTumorIndex >= 0) {
					MutTumor = split[MutTumorIndex];					
				}
				
				int mut_count = new Integer(MutTumor);
				...
				if (!split[1].contains("SJL") && manReviewFlag.equals("Good") && mut_count >= 5 && (split[5].toUpperCase().equals(muttype.toUpperCase()) || muttype.toUpperCase().equals("ALL"))) {
					...
					if (map.containsKey(sampleName)) {
						...
						out.write(str + "\t" + mut_type + "\t" + sampleType + "\t" + transition_or_transversion + "\t" + sampleType + "_" + split[2] + "\n");
	...
	public static void generateBasicTable(String inputFile, String sampleFile, String outputFile, String outputChromosomeFile) {
		try {
			...
			while (in.ready()) {
				...
				if (quality.contains("SJH") && manReviewFlag.equals("Good")) {
