...
public class CalculateWilcoxon {
	...
	public static String description() {
		return "Wilcoxon test between two groups.";
	}
	...
	public static void execute(String[] args) {
		try {
			...
			generateWilcoxonResult(inputFile, outputAllFile, outputUpRegFile, outputDnRegFile, groupA, groupB, pvalCutoff, logFC, takeLog);
			
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	public static void generateWilcoxonResult(String inputFile, String outputAllFile, String outputUpRegFile, String outputDnRegFile, int[] index1, int[] index2, double pval, double foldChange, boolean takeLog) {
		...
		try {
			...
			while (in.ready()) {
				...
				double wilcoxon_pval = mannWhitney.mannWhitneyUTest(d1, d2);
				...
				list.add(split[0] + "\t" + (group1 - group2) + "\t" + all_mean + "\t" + wilcoxon_pval);
	...
	public static void calculateWilcoxTest(String inputFile, String outputFile, int[] index1, int[] index2) {
		WilcoxonSignedRankTest wilcoxTest = new WilcoxonSignedRankTest();
		MannWhitneyUTest mannWhitney = new MannWhitneyUTest();
		try {
			...
			while (in.ready()) {
				...
				double group2 = MathTools.mean(d2);
				double wilcoxon_pval = mannWhitney.mannWhitneyUTest(d1, d2);
				out.write(split[0] + "\t" + group1 + "\t" + group2 + "\t" + (group1 - group2) + "\t" + wilcoxon_pval + "\n");
