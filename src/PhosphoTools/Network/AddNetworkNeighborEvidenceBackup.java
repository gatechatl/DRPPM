 ...
 *
 */
public class AddNetworkNeighborEvidenceBackup {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			try {
				...
				HashMap predicted_list = new HashMap(); // generate a predicted list
				...
				//System.out.println("Finished Loading Predicted_List: " + count);
				...
				while (in.ready()) {
					...
					if (count % 10000 == 0) {
						System.out.println((new Double(new Double(count) / total_count * 100)).intValue() + "% Finished");
					}
					count++;
					String[] split = str.split("\t");
					String uniprot = split[0].split("\\|")[1];
					...
					if (uniprot2geneID.containsKey(uniprot)) {
						...
						if (motifName2kinase.containsKey(motif_name)) {
							...
							for (String kinase: kinases.split(",")) {
								kinase = kinase.toUpperCase();			
								LinkedList predicted_gene_list = (LinkedList)predicted_list.get(kinase);
								...
								if (interactiveDB.containsKey(geneID)) {
									...
									while (itr.hasNext()) {
										...
										if (predicted_gene_list.contains(local_sub_gene.toUpperCase())) {
											found_local_predicted_substrate = true;
											found_local_predicted_substrates.add(local_sub_gene.toUpperCase());
										}
									}
									...
									if (found_local_substrate) {
										...
										if (checkIfPresentAlready) {
											...
											//}
											out.write(str + "\t" + kinase + "\t" + kinase + ":present_found_neighbor-" + totalList + "\n");
										} else {
											out.write(str + "\t" + kinase + "\t" + kinase + ":found_neighbor-" + totalList + "\n");
											if (motif_statistics.containsKey(key)) {
												LinkedList list = (LinkedList)motif_statistics.get(key);
										...
										if (checkIfPresentAlready) {
											...
											out.write(str + "\t" + kinase + "\t" + kinase + ":found_predicted_neighbor\n");
										}
									} else {
										if (checkIfPresentAlready) {
											...
											//}
											out.write(str + "\t" + kinase + "\t" + kinase + ":present_but_no_neighbor\n");
											...
											out.write(str + "\t" + kinase + "\t" + kinase + ":no_neighbor\n");
											if (motif_statistics.containsKey(key)) {
												LinkedList list = (LinkedList)motif_statistics.get(key);
									...
									if (checkIfPresentAlready) {
										out.write(str + "\t" + kinase + "\t" + kinase + ":present_but_missing_in_interactiveDB\n");
										if (motif_statistics.containsKey(key)) {
											LinkedList list = (LinkedList)motif_statistics.get(key);
										...
										out.write(str + "\t" + kinase + "\t" + kinase + ":missing_in_interactiveDB\n");
										if (motif_statistics.containsKey(key)) {
											LinkedList list = (LinkedList)motif_statistics.get(key);
						...
						out.write(str + "\t" + "NA" + "\tMissingUniprot2GeneID\n");
					}
				}
				in.close();
				out.close();
				...
				while (itr4.hasNext()) {
					...
					} else if (list.contains("present_found_predicted_neighbor")) {
				...
				double proportionFoundPredicted = new Double((new Double(countPredicted / uniq_protein_phosite.size() * 10000)).intValue()) / 100;
				...
				System.out.println("Present in Kin-Sub DB and DB annotated its neighbor also substrate for same Kinase: " + present_found_neighbor);
				summary_out.write("Present in Kin-Sub DB and DB annotated its neighbor also subtrate for same Kinase: " + present_found_neighbor + "\n");
				System.out.println("Present in Kin-Sub DB and neighbor is also subtr for same Kinase based on prediction: " + present_found_predicted_neighbor);
				summary_out.write("Present in Kin-Sub DB and neighbor is also subtr for same Kinase based on prediction: " + present_found_predicted_neighbor + "\n");
				System.out.println("Present in Kin-Sub DB but its neighbor is not: " + present_but_no_neighbor);							
				summary_out.write("Present in Kin-Sub DB but its neighbor is not: " + present_but_no_neighbor + "\n");
				System.out.println("* Putative Kin-Sub. Its neighbor is substrate of same kinase based on DB: " + found_neighbor);
				summary_out.write("* Putative Kin-Sub. Its neighbor is substrate of same kinase based on DB: " + found_neighbor + "\n");
				System.out.println("* Putative Kin-Sub. Its neighbor is also substr for same kinase based on prediction: " + found_predicted_neighbor);
				summary_out.write("* Putative Kin-Sub. Its neighbor is also substr for same kinase based on prediction: " + found_predicted_neighbor + "\n");
				System.out.println("Substrate is part of interactiveDB, but none of its neighbor has same kinase-substr: " + no_neighbor);
				summary_out.write("Substrate is part of interactiveDB, but none of its neighbor has same kinase-substr: " + no_neighbor + "\n");
				System.out.println("Present in Kin-Sub DB but it is not part of interactiveDB: " + present_but_missing_in_interactiveDB);
				summary_out.write("Present in Kin-Sub DB but it is not part of interactiveDB: " + present_but_missing_in_interactiveDB + "\n");
				System.out.println("Substrate is absent from interactiveDB: " + missing_in_interactiveDB);
				summary_out.write("Substrate is absent from interactiveDB: " + missing_in_interactiveDB + "\n");
				//System.out.println("Other: " + other);
				//summary_out.write("Other: " + other + "\n");
				
				System.out.println("\n#### Estimated Sensitivity and Specificity ####");
				summary_out.write("\n####Estimated Sensitivity and Specificity ####" + "\n");				
				System.out.println("Percentage of Phosphosite With Predicted Kinase-Substrate: " + proportionFoundPredicted + "%");
				summary_out.write("Percentage of Phosphosite With Predicted Kinase-Substrate: " + proportionFoundPredicted + "%" + "\n");
				System.out.println("Sensitivity: " + new Double(present_found_neighbor) / (present_found_neighbor + present_but_no_neighbor));
				summary_out.write("Sensitivity: " + new Double(present_found_neighbor) / (present_found_neighbor + present_but_no_neighbor) + "\n");
				System.out.println("Specificity: " + new Double(count_neg_not_found) / (count_neg_found + count_neg_not_found));
				summary_out.write("Specificity: " + new Double(count_neg_not_found) / (count_neg_found + count_neg_not_found) + "\n");
				summary_out.close();
