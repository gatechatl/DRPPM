...
public class KinaseSubstrate2KinaseOnly {
	...
	public static String description() {
		return "Generate kinase substrate relationship regardless of activity site\n This version works with older files";
	}

	public static String parameter_info() {
		return "[kinaseSubstrateFileName] [kinaseListFile] [activeSiteFile] [inhibitSiteFile] [allSiteFile] [pathwayGeneList] [outputFile]";
	}
	...
	public static void execute(String[] args) {
		...
		try {
			...
			String activitySiteFile = args[2];
			...
			fstream = new FileInputStream(activitySiteFile);
			...
			while (in.ready()) {
				...
				String[] split = str.split("\t");
				if (split.length > 10) {
					String kinase1 = split[0].toUpperCase();					
					String substrate = split[6].toUpperCase();
					String accession = split[3];
					String site = split[4];
					if (accession.equals("CONVERT_UNIPROT")) {
						accession = split[5];
						site = split[8];
					}
					...
					//
					boolean found = false;
					if (((pathwayGeneList.containsKey(kinase1) && pathwayGeneList
							.containsKey(substrate)) || pathwayGeneList.size() == 0)
							&& kinase.containsKey(substrate)
							&& activeSite.containsKey(key)) {
						found = true;
					...
					if (!found) {
						if (((pathwayGeneList.containsKey(kinase1) && pathwayGeneList
								.containsKey(substrate)) || pathwayGeneList
								.size() == 0)
								&& kinase.containsKey(substrate)
								&& allSite.containsKey(key)) {
							found = true;
							String tag = kinase1 + "\tPhosphorylationDependentFunction\t" + substrate + "\t" + accession + "_" + site;;
							relationship.put(tag, tag);
						}
					}
					if (!found) {
