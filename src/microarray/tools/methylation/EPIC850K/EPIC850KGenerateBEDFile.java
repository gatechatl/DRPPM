...
public class EPIC850KGenerateBEDFile {
	...
	public static String parameter_info() {
		return "[inputFile] [refFile] [sampleNameFile] [outputFile]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			Iterator itr_samples = sampleName.iterator();
			while (itr_samples.hasNext()) {
				String sample = (String)itr_samples.next();
				
				FileWriter fwriter = new FileWriter(sample + ".bedgraph");
				BufferedWriter out = new BufferedWriter(fwriter);
				System.out.println("Writing: " + sample + ".bedgraph");
				...
				out.write(header1 + sample + header2 + "\n");
				...
				for (int i = 0; i < header_split.length; i++) {
					...
					if (header_split[i].contains(sample) && header_split[i].contains("AVG_Beta")) {
