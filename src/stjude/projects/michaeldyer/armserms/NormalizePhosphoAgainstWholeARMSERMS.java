 ...
 *
 */
public class NormalizePhosphoAgainstWholeARMSERMS {
	...
	public static String parameter_info() {
		return "[phosphoFile] [wholeFile] [outputFile] [kinase annotation flag true/false]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			boolean kinase_annotation_flag = new Boolean(args[3]);
			...
			while (in.ready()) {
				...
				if (map.containsKey(geneName)) {
					...
					double[] pho = new double[30];
					int j = 0;
					if (kinase_annotation_flag) {
						for (int i = split.length - 31; i < split.length - 1; i++) {
					...
					j = 0;
					for (int i = split2.length - 30; i < split2.length; i++) {
						double norm = pho[j] - new Double(split2[i]);
						j++;
						out.write("\t" + norm);
					}
