...
public class XiangChenGrabTopVariableGenesFilterSNPXY {
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				if (split.length > 11) {
					chr = split[11];
					if (chr.toUpperCase().contains("X")) {
						filter_id2snp_sexchr.put(name, name);
					}
					if (chr.toUpperCase().contains("Y")) {
						filter_id2snp_sexchr.put(name, name);
					}
					if (chr.toUpperCase().equals("")) {
						filter_id2snp_sexchr.put(name, name);
					}
				} else {
					// if it is less than 11 meaning it didn't contain chromosome information
					filter_id2snp_sexchr.put(name, name);
				}
				if (split.length > 45) {
					//String name = split[1];
					String snp_id = split[43];
					String snp_freq = split[45];
					//String chr = split[11];
					boolean freq_larger_001 = false;
					if (!snp_freq.equals("")) {
						for (String freq: snp_freq.split(";")) {
							if (new Double(freq) >= 0.01) {
								freq_larger_001 = true;
							}
						}
					}
					if (freq_larger_001 || chr.toUpperCase().equals("X") || chr.toUpperCase().equals("Y")) {
						filter_id2snp_sexchr.put(name, snp_id); // this list will be used for filtering in the next step
			...
			while (in.ready()) {
				...
				boolean skip = false;
				if (!str.contains("NaN") && !filter_id2snp_sexchr.containsKey(split[0])) {
					if (split[0].equals("cg13267486")) {
						System.out.println("Something is wrong: cg13267486");
					}
				//if (!str.contains("NaN")) {
					String line = "";
					//out.write(split[0]);
