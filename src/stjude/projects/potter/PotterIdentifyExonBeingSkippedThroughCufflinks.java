...
public class PotterIdentifyExonBeingSkippedThroughCufflinks {

	public static String description() {
		return "Potter identify exon that is being skipped through cufflinks..";
	}
	public static String type() {
		return "MISC";
	}
	public static String parameter_info() {
		return "[Gang Junction File] [transcriptFileList: three column (transcript.gtf) (output exon sumary) (transcript list)]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			String transcriptFileList = args[1]; // three column (transcript.gtf) (output exon sumary) (transcript list)
			...
			FileInputStream fstream3 = new FileInputStream(transcriptFileList);
			...
			while (in3.ready()) {
				...
				if (!inputFileStr.trim().equals("")) {
					String inputFile2 = inputFileStr.split("\t")[0].trim();
					String outputFile = inputFileStr.split("\t")[1].trim();
					String outputFile2 = inputFileStr.split("\t")[2].trim();
					...
					//inputFile2 = "\\\\gsc.stjude.org\\project_space\\pottegrp\\SF3B1\\common\\CMPB\\BioinfoCore\\Hela_Rh18\\Cufflinks\\" + file + "\\transcripts.gtf";
					...
					//String outputFile = "\\\\gsc.stjude.org\\project_space\\pottegrp\\SF3B1\\common\\CMPB\\BioinfoCore\\Hela_Rh18\\Cufflinks\\" + file + "_output_result.txt";
					...
					//String outputFile2 = "\\\\gsc.stjude.org\\project_space\\pottegrp\\SF3B1\\common\\CMPB\\BioinfoCore\\Hela_Rh18\\Cufflinks\\" + file + "_tally_of_junctions.txt";
					...
					//inputFile = "\\\\gsc.stjude.org\\project_space\\pottegrp\\SF3B1\\common\\CMPB\\BioinfoCore\\Hela_Rh18\\junction\\SUD_vs_DMSO_diff_novel_junctions_upclassified.txt";
					...
					while (in.ready()) {
						...
						HashMap first_hit = new HashMap();
						HashMap overlap_hit =new HashMap();
						if (keep_track_start_hits.containsKey(split[0])) {
							...
							while (itr.hasNext()) {
								...
								first_hit.put(transcript_id.trim(), transcript_id.trim());
							}
							out2.write("\ttrue1\t" + transcript);
						} else {
							out2.write("\tfalse\tNA");
						}
						if (keep_track_end_hits.containsKey(split[0])) {
							...
							while (itr.hasNext()) {
								String transcript_id = (String)itr.next();
								transcript += transcript_id + ",";
								if (first_hit.containsKey(transcript_id)) {
									overlap_hit.put(transcript_id, transcript_id);
								}
							}
							out2.write("\ttrue2\t" + transcript);
						} else {
							out2.write("\tfalse\tNA");
						}
						out2.write("\t");
						Iterator itr2 = overlap_hit.keySet().iterator();
						while (itr2.hasNext()) {
							String hits = (String)itr2.next();
							out2.write(hits + ",");
						}
						out2.write("\n");
					}
