...
public class MondiraKunduStressGranuleCalculateOverlap {
	...
	public static void main(String[] args) {
		...
		try {
			...
			inputFile = "\\\\gsc.stjude.org\\project_space\\kundugrp\\ULK\\common\\ULK1_PROTEOMICS\\ULK1_PPI\\COMBINED_STRESSGRANULE\\StressGranule.txt";
			...
			in.close();
			String outputFile_ULK1_interactome = "\\\\gsc.stjude.org\\project_space\\kundugrp\\ULK\\common\\ULK1_PROTEOMICS\\ULK1_PPI\\COMBINED_STRESSGRANULE\\ULK1_Overlap_StressGranule_Target.txt";
			...
			String outputFile_stressgranule = "\\\\gsc.stjude.org\\project_space\\kundugrp\\ULK\\common\\ULK1_PROTEOMICS\\ULK1_PPI\\COMBINED_STRESSGRANULE\\ULK1_Overlap_StressGranule_DB.txt";
			FileWriter fwriter_stressgranule = new FileWriter(outputFile_stressgranule);
			BufferedWriter out_stressgranule = new BufferedWriter(fwriter_stressgranule);
			...
			itr = stress_granule.keySet().iterator();
			while (itr.hasNext()) {
				String gene = (String)itr.next();
				out_stressgranule.write(gene);
				if (target.containsKey(gene)) {
					hit.put(gene, gene);
					out_stressgranule.write("\ttrue\n");
				} else {
					out_stressgranule.write("\tfalse\n");
				}
			}
			System.out.println("Total Background: " + background.size());
			System.out.println("Total Stress Granule: " + stress_granule.size());
			System.out.println("Total Target: " + target.size());			
			System.out.println("Total Hit: " + hit.size());
			out_ULK1_interactome.close();
			out_stressgranule.close();
