...
public class JinghuiZhangTCRBCRScoreSurvivalForWLM {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String inputPatientFile = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Survival_Analysis\\SJWLM_TARGET\\TARGET_WLM_Patient_Metadata.txt";			
			...
			while (in.ready()) {
				...
				for (int i = 1; i < split.length; i++) {
					if (split_header[i].contains("SJWLM")) {
			...
			String outputFile = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Survival_Analysis\\SJWLM_TARGET\\TARGET_WLM_Patient_Metadata_TCRBCR.txt";
			FileWriter fwriter = new FileWriter(outputFile);
			BufferedWriter out = new BufferedWriter(fwriter);

			String outputFile_script = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Survival_Analysis\\SJWLM_TARGET\\tcr_bcr_script.r";
			FileWriter fwriter_script = new FileWriter(outputFile_script);
			BufferedWriter out_script = new BufferedWriter(fwriter_script);

			inputPatientFile = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Survival_Analysis\\SJWLM_TARGET\\TARGET_WLM_Patient_Metadata.txt";			
			fstream = new FileInputStream(inputPatientFile);
			din = new DataInputStream(fstream);
			in = new BufferedReader(new InputStreamReader(din));
			header = in.readLine();
			...
			while (itr.hasNext()) {
				...
				out_script.write("res.cox_wlm <- coxph(Surv(Overall_Survival_Time_in_Days, VitalStatus) ~ " + feature + ", data = data_wlm)\n");
				out_script.write("pval = summary(res.cox_wlm)$logtest[3]\n");
				out_script.write("write(paste('" + feature + "', 'OS',pval, sep=\" \"), file = \"\\\\\\\\gsc.stjude.org\\\\project_space\\\\zhanggrp\\\\AltSpliceAtlas\\\\common\\\\analysis\\\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\\\Survival_Analysis\\\\SJWLM_TARGET\\\\TCR_BCR_output.txt\", append = TRUE)\n\n");
				out_script.write("res.cox_wlm <- coxph(Surv(Event_Free_Survival_Time_in_Days, First_Event == \"Relapse\") ~ " + feature + ", data = data_wlm)\n");
				out_script.write("pval = summary(res.cox_wlm)$logtest[3]\n");
				out_script.write("write(paste('" + feature + "', 'First_Event_Relapse',pval, sep=\" \"), file = \"\\\\\\\\gsc.stjude.org\\\\project_space\\\\zhanggrp\\\\AltSpliceAtlas\\\\common\\\\analysis\\\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\\\Survival_Analysis\\\\SJWLM_TARGET\\\\TCR_BCR_output.txt\", append = TRUE)\n\n");
			...
			while (in.ready()) {
				...
				if (patient2sjid.containsKey(patient_id)) {
					...
					if (sampleNames.containsKey(sjid) && sjid.contains("SJWLM")) {
