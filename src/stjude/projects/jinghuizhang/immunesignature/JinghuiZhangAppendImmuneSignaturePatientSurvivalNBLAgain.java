...
public class JinghuiZhangAppendImmuneSignaturePatientSurvivalNBLAgain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			in = new BufferedReader(new InputStreamReader(din));
			header = in.readLine();
			out.write(header + "\t" + "ImmuneSubtypeStr\tImmuneSubtype\tVitalStatus\tFirstEvent\tSJID\tSCNA\tHyperDiploid\tImmuneSuppressed\tCluster1\tCluster2\tCluster3\tCluster4\tCluster5");
			...
			while (in.ready()) {
				...
				if (patient2sjid.containsKey(patient_id)) {
					...
					if (immune_subtype_map.containsKey(sjid)) {
						...
						if (immune_subtype_str.contains("1")) {
							cluster1 = "1";
						}
						if (immune_subtype_str.contains("2")) {
							cluster2 = "1";
						}
						if (immune_subtype_str.contains("3")) {
							cluster3 = "1";
						}
						if (immune_subtype_str.contains("4")) {
							cluster4 = "1";
						}
						if (immune_subtype_str.contains("5")) {
							cluster5 = "1";
						}
						if (immune_subtype_str.contains("6")) {
							cluster6 = "1";
						}
						...
						out.write(str + "\t" + immune_subtype_str + "\t" + immune_activity + "\t" + vital_status + "\t" + first_event + "\t" + sjid + "\t" + scna_status + "\t" + hyperdiploid_status + "\t" + immune_suppressed + "\t" + cluster1 + "\t" + cluster2 + "\t" + cluster3 + "\t" + cluster4 + "\t" + cluster5);
