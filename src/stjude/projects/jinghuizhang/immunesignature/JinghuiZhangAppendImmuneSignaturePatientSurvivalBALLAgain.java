...
public class JinghuiZhangAppendImmuneSignaturePatientSurvivalBALLAgain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String inputPatientFile = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis\\Survival_Analysis\\SJBALL_TARGET\\TARGET_BALL_Patient_Metadata.txt";			
			...
			while (in.ready()) {
				...
				for (int i = 1; i < split.length; i++) {
					if (split_header[i].contains("SJBALL")) {
			...
			String outputFile = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Survival_Analysis\\SJBALL_TARGET\\TARGET_BALL_Patient_Metadata_ImmuneSubtype.txt";
			FileWriter fwriter = new FileWriter(outputFile);
			BufferedWriter out = new BufferedWriter(fwriter);

			String outputFile_script = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Survival_Analysis\\SJBALL_TARGET\\script.r";
			FileWriter fwriter_script = new FileWriter(outputFile_script);
			BufferedWriter out_script = new BufferedWriter(fwriter_script);

			inputPatientFile = "Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Survival_Analysis\\SJBALL_TARGET\\TARGET_BALL_Patient_Metadata.txt";			
			fstream = new FileInputStream(inputPatientFile);
			din = new DataInputStream(fstream);
			in = new BufferedReader(new InputStreamReader(din));
			header = in.readLine();
			out.write(header + "\t" + "ImmuneSubtypeStr\tImmuneSubtype\tVitalStatus\tFirstEvent\tSJID\tImmuneSuppressed");
			...
			while (itr.hasNext()) {
				...
				out_script.write("res.cox_ball <- coxph(Surv(Overall_Survival_Time_in_Days, VitalStatus) ~ " + feature + ", data = data_ball)\n");
				out_script.write("pval = summary(res.cox_ball)$logtest[3]\n");
				out_script.write("write(paste('" + feature + "', 'OS',pval, sep=\" \"), file = \"\\\\\\\\gsc.stjude.org\\\\project_space\\\\zhanggrp\\\\AltSpliceAtlas\\\\common\\\\analysis\\\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\\\Survival_Analysis\\\\SJBALL_TARGET\\\\output.txt\", append = TRUE)\n\n");
				out_script.write("res.cox_ball <- coxph(Surv(Event_Free_Survival_Time_in_Days, First_Event == \"Relapse\") ~ " + feature + ", data = data_ball)\n");
				out_script.write("pval = summary(res.cox_ball)$logtest[3]\n");
				out_script.write("write(paste('" + feature + "', 'First_Event_Relapse',pval, sep=\" \"), file = \"\\\\\\\\gsc.stjude.org\\\\project_space\\\\zhanggrp\\\\AltSpliceAtlas\\\\common\\\\analysis\\\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\\\Survival_Analysis\\\\SJBALL_TARGET\\\\output.txt\", append = TRUE)\n\n");
			...
			while (in.ready()) {
				...
				if (patient2sjid.containsKey(patient_id)) {
					...
					if (immune_subtype_map.containsKey(sjid)) {
						...
						out.write(str + "\t" + immune_subtype_str + "\t" + immune_activity + "\t" + vital_status + "\t" + first_event + "\t" + sjid + "\t" + immune_suppressed);
