 ...
 *
 */
public class JinghuiZhangTCRBCRScoreWithSNVINDEL {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String outputFile_ssGSEA = "\\\\gsc.stjude.org\\project_space\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Uncorrected\\SpearmanCorrelMutationBurden\\pcgp_TCRBCR_filtered2SNVINDEL_ALL.txt";
			FileWriter fwriter = new FileWriter(outputFile_ssGSEA);
			BufferedWriter out = new BufferedWriter(fwriter);
			
			String outputFile_SNVINDEL_Count = "\\\\gsc.stjude.org\\project_space\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Uncorrected\\SpearmanCorrelMutationBurden\\PCGP_TARGET_TCRBCR_SNVINDEL_Table_Prioritized_Filtered_ALL.txt";
			...
			String outputFile_Disease_Separman = "\\\\gsc.stjude.org\\project_space\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\ImmuneSignatureAnalysis_Updated_Matrix_20200518\\Uncorrected\\SpearmanCorrelMutationBurden\\PCGP_TARGET_TCRBCR_SNVINDEL_Table_Disease_Spearman_ALL.txt";
			...
			HashMap map_tcr_bcr_score = new HashMap();
			String inputFile = "\\\\gsc.stjude.org\\project_space\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\TRUST4\\combined_TCR_BCR_TRUST4.output.txt"; 		
			...
			while (in.ready()) {
				...
				map_tcr_bcr_score.put(split[0], str);
			}
			in.close();
			...
			Iterator itr = map_tcr_bcr_score.keySet().iterator();
			while (itr.hasNext()) {
				String sampleName = (String)itr.next();
				if (prioritized_SNVINDEL.containsKey(sampleName)) {
					String line = (String)map_tcr_bcr_score.get(sampleName);
			...
			while (itr.hasNext()) {
				String sampleName = (String)itr.next();
				if (map_tcr_bcr_score.containsKey(sampleName)) {
