...
public class JinghuiZhangHarmonizeCandidatesFinal {
	...
	public static String parameter_info() {
		return "[inputCandidateFile] [inputProteomicHitFile] [inputTranslationFile] [outputFile] [outputFile_PanCan]";
	}
	public static void execute(String[] args) {
		
		try {
			String inputCandidateFile = args[0]; //"Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\Comprehensive_CAR-T_Analysis\\hg38_analysis\\AfterLiqingsExonCounting\\PCGP_TARGET_GTEx_Filtered_Candidates_20200212.txt";
			...
			String outputFile = args[3]; ///"Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\Comprehensive_CAR-T_Analysis\\hg38_analysis\\AfterLiqingsExonCounting\\PCGP_TARGET_GTEx_For_HTML.txt";
			String outputFile_PanCan = args[4]; //"Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\Comprehensive_CAR-T_Analysis\\hg38_analysis\\AfterLiqingsExonCounting\\PCGP_TARGET_GTEx_For_HTML_PanCan.txt";
			
			FileWriter fwriter = new FileWriter(outputFile);
			BufferedWriter out = new BufferedWriter(fwriter);
			out.write("ExonID\tGTEx\tPediatric_Cancer\tgeneSymbol\thg38_pos\tProteinAccession\tPeptide\tRMS_Proteomics_Validation\tCandidateFlag\tExpressedDisease\tNormal_Some_Expression\tNormal_Above_Medium_Expr\tBlank\n");

			
			
			FileWriter fwriter_PanCan = new FileWriter(outputFile_PanCan);
			BufferedWriter out_PanCan = new BufferedWriter(fwriter_PanCan);
			out_PanCan.write("ExonID\tGTEx\tPediatric_Cancer\tgeneSymbol\thg38_pos\tProteinAccession\tPeptide\tRMS_Proteomics_Validation\tCandidateFlag\tExpressedDisease\tNormal_Some_Expression\tNormal_Above_Medium_Expr\tBlank\n");
			...
			while (in.ready()) {
				...
				if (new Integer(end) - new Integer(start) >= 5) {
					...
					gtex_score = gtex_score / (74 - 45 + 1);
					boolean rms_validation_flag = false;
					if (rms_validation.containsKey(exon_name)) {
						rms_validation_flag = true;
					}
					String hg38_pos = chr + ":" + start + ":" + end + ":" + strand;
					out.write(exon_name + "\t" + gtex_score + "\t" + pcgp_target_score + "\t" + geneName + "\t" + hg38_pos + "\t" + protein_accession + "\t" + peptide + "\t" + rms_validation_flag + "\t" + candidate + "\t" + highly_expressed_disease + "\t" + normal_tissue_some + "\t" + normal_tissue_median + "\tblank\n");
