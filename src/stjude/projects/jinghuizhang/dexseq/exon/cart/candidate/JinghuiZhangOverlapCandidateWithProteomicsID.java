 ...
 *
 */
public class JinghuiZhangOverlapCandidateWithProteomicsID {

	public static String description() {
		return "Overlap bed candidates with proteomics";
	}
	public static String type() {
		return "JinghuiZhang";
	}
	public static String parameter_info() {
		return "[inputCandidateFile] [ID_inputFile] [outputFile]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			int candidates = 0;
			String inputCandidateFile = args[0]; //"Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\Comprehensive_CAR-T_Analysis\\hg38_analysis\\AfterLiqingsExonCounting\\Summary\\candidate_peptide.txt"; //"Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\Comprehensive_CAR-T_Analysis\\hg38_analysis\\AfterLiqingsExonCounting\\Summary\\PCGP_TARGET_GTEx_Candidates.bed"; // args[0];
			String ID_inputFile = args[1]; //"\\\\gsc.stjude.org\\project_space\\penggrp\\ProteomicsRMS\\common\\HongWangProteomics\\Combined_ID\\Combined_ID.txt";
			String outputFile = args[2]; //"Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\Comprehensive_CAR-T_Analysis\\hg38_analysis\\AfterLiqingsExonCounting\\Summary\\candidate_peptide_with_peptide_support.txt"; //"Z:\\ResearchHome\\ProjectSpace\\zhanggrp\\AltSpliceAtlas\\common\\analysis\\Comprehensive_CAR-T_Analysis\\hg38_analysis\\AfterLiqingsExonCounting\\Summary\\PCGP_TARGET_GTEx_Candidates.bed"; // args[0];
			...
			while (in.ready()) {
				...
				if (split.length > 5) {
					String protein_id = split[2];
					String peptide = split[5];
					if (peptide.length() > 7) {
						candidates++;
						if (map_hits.containsKey(protein_id)) {
							HashMap peptides = (HashMap)map_hits.get(protein_id);
							peptides.put(protein_id + "\t" + split[3] + "\t" + split[4], str);
			...
			fstream = new FileInputStream(ID_inputFile);
