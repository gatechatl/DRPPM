...
/**
 * Calculate the exon expression max for each sample type.
 ...
 *
 */
public class JinghuiZhangCalculateSampleTypeExonExpressionMax {
	public static String description() {
		return "Calculate the exon expression max for each sample type.";
	}
	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				geneID2metaInfo.put(split[1], geneID + "\t" + geneName + "\t" + type + "\t" + status + "\t" + chr + "\t" + start + "\t" + end);					
			...
			out.write("\tgeneID\tgeneName\ttype\tstatus\tchr\tstart\tend\n");
				...
				if (geneID2metaInfo.containsKey(split[0])) {
					...
					while (itr.hasNext()) {
						...
						if (disease_values.length > 0) {
							double tissue_median = MathTools.max(disease_values);
