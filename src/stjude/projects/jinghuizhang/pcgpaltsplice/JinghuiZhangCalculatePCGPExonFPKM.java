 ...
 *
 */
public class JinghuiZhangCalculatePCGPExonFPKM {
	...
	public static String parameter_info() {
		return "[input sampleFile list] [path of comprehensive folder] [inputCustomGeneIDInfo] [outputFile]";
	}
			...
			while (in.ready()) {
				...
				if (f.exists()) {
					...
					while (itr2.hasNext()) {
						...
						double length = (Double)geneID2length.get(exon_id);
						
						double fpkm = new Double(0.0001);
						if (values.containsKey(exon_id)) {
							fpkm = (((Double)values.get(exon_id) + 0.0001) * 1000 * 1000000) / (length * total);
							//double FN1_EBD_fpkm = (((fn1_splice_count + 0.0001) * 1000 * 1000000) / (exon_length * total));
						}
						fpkm = new Double(new Double(fpkm * 1000).intValue()) / 1000;
						if (fpkm <= 0.0001) {
							fpkm = 0.0001;
						}
