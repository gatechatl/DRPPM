 ...
 *
 */
public class JinghuiZhangSTARMappingFromYaweiSingleEnd {
	...
	public static String parameter_info() {
		return "[inputLstFile] [star_index_dir] [num_thread] [outputFile] [optional: gzip flag, default is false]";
	}
	public static void execute(String[] args) {
		try {
			...
			while (in.ready()) {
				...
				if (split.length == 2) {
					String script = generate_star_script(star_index_dir, split[0], split[1], num_thread, gzip_flag);
					CommandLine.writeFile(split[1] + ".sh", script);
					out.write("sh " + split[1] + ".sh\n");
				} else {
					System.out.println("Expecting only two column: fastq_path\tsampleName");
				}
			}
			in.close();
			out.close();
	...
	public static String generate_star_script(String genomeDir, String inputR1, String prefix, String thread, boolean gzip_flag) {
		String line = "";
		line += "mkdir " + prefix + " \n";
		line += "cd " + prefix + " \n";
		line += "\n";
