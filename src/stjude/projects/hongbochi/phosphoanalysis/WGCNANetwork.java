	...
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				String kinases = split[11];
				if (!kinases.equals("NA")) {
					String[] split2 = kinases.split(",");
					for (String kinase: split2) {
						if (kinase_map.containsKey(kinase)) {
							LinkedList list = (LinkedList)kinase_map.get(kinase);
							if (!list.contains(name)) {
								list.add(name);
							}
							kinase_map.put(kinase, list);
						} else {
			...
			HashMap accession2gene = new HashMap();
				...
				while (itr2.hasNext()) {						
					...
					if (peptide2accession.containsKey(new_peptide)) {						
						...
						while (itr.hasNext()) {
							...
							accession2gene.put(accession, geneName);
			...
			while (itr.hasNext()) {
				...
				int hit = 0;				
				LinkedList list = (LinkedList)kinase_map.get(kinaseName);
				int totalKinase = list.size();
				Iterator itr2 = list.iterator();
				while (itr2.hasNext()) {
					String accession = (String)itr2.next();					
					if (phosphosite.containsKey(accession)) {
						String geneName = (String)accession2gene.get(accession);
						if (!combo.containsKey(kinaseName.toUpperCase() + "\t" + geneName.toUpperCase())) {
							out.write(kinaseName.toUpperCase() + "\tconnect\t" + geneName.toUpperCase() + "\n");
							combo.put(kinaseName.toUpperCase() + "\t" + geneName.toUpperCase(), "");
						}
						hit++;
					} 
				}
				
				//out.write(kinaseName + "\t" + fisher_pvalue + "\t" + corrected_fisher_pvalue + "\t" + enrichment_value + "\t" + a + "\t" + b + "\t" + c + "\t" + d + "\n");
			}
