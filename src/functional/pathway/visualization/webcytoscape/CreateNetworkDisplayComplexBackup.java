...
public class CreateNetworkDisplayComplexBackup {
...
public class CreateNetworkDisplayComplexBackup {
	...
	public static String generateNodesComplex(LinkedList kegg_conn_list, LinkedList kegg_list, HashMap expression, HashMap rawExpression, String networkType, int fontSize) {
		...
		while (itr.hasNext()) {
			KEGGentry entry = (KEGGentry)itr.next();
			String key = entry.SHORT_NAME;
			String node_color = entry.COLOUR;
		...
		while (itr.hasNext()) {
			...
			if (kegg_list.size() != count) {				
				str += "{ data: { id: '" + key + "', name: '" + key + "(" + expr + ")" + "', genes: '" + genes + "', weight: " + weight + ", faveShape: '" + faveShape + "', baz: " + size + ", faveColor: '" + bcolor + "', bwidth: " + bwidth + "}, position:{x: " + new Double(new Double(entry.XCOORD) * 1.5).intValue() + ",y: " + new Double(new Double(entry.YCOORD) * 1.5).intValue() + "}},\n";
			} else {
				str += "{ data: { id: '" + key + "', name: '" + key + "(" + expr + ")" + "', genes: '" + genes + "', weight: " + weight + ", faveShape: '" + faveShape + "', baz: " + size + ", faveColor: '" + bcolor + "', bwidth: " + bwidth + "}, position:{x: " + new Double(new Double(entry.XCOORD) * 1.5).intValue() + ",y: " + new Double(new Double(entry.YCOORD) * 1.5).intValue() + "}}\n";
				//str += "{ data: { id: '" + key + "', weight: 50, faveShape: 'rectangle', baz: " + size + "} }\n";
			}
