 ...
 *
 */
public class GenerateMultipleCirclesFlex {
	...
	public static String parameter_info() {
		return "[moduleInformation: two column (module name, gene list separated by coma)] [moduleLayout: ModuleName, Xcoord, Ycoord, HexColor, size (0-1)]";
	}
	public static void execute(String[] args) {
		...
		try {
			...
			while (in.ready()) {
				...
				String[] split = str.split("\t");
				String colour = "white";
				double radius_size = radius;
				if (split.length > 3) {
					colour = split[3];
				}
				if (split.length > 4) {
					radius_size = new Double(split[4]);
				}
				mod_layout.put(split[0], split[1] + "\t" + split[2] + "\t" + colour + "\t" + radius_size);
			}
			...
			while (itr.hasNext()) {
				...
				double radius_size = new Double(layout_coord_str.split("\t")[3]);
				LinkedList list = (LinkedList)map.get(moduleName);
				HashMap coord = generateCoordinates(list, radius_size);
				...
				while (itr2.hasNext()) {
					...
					System.out.println(geneName + "\t" + 10 + "\t" + "black" + "\t" + color_text + "\t" + xaxis + "\t" + yaxis + "\t" + "ellipse" + "\t" + "0.0" + "\t" + "100");
				}
			}
	...
	public static HashMap generateCoordinates(LinkedList nodes, double radius_buffer) {
		HashMap map = new HashMap();
		...
		int x_next, y_next;
		double radius = (nodes.size() * 20 + 300) * radius_buffer;
